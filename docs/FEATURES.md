...
- **What:** Weave sidebar **Load-in** group: **Diagonal load-in** slider (0–1) scrubs the default diagonal wave reveal (`u_revealProgress` in `fragment.glsl`). Auto-plays on load and when the weave pattern changes; drag the slider to pull the wave back. URL **`drp`** (0–1000 → 0–1; omitted when fully revealed) and JSON **`state.diagonalRevealProgress`** / **`playing.diagonalReveal.progress`** preserve scrubbed frames in share links and handoff. **Why:** Same opening motion as before, with manual scrub for previews and framing; share links restore an exact wave position instead of replaying on open.
