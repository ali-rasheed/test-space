...
- **What:** Added weave draft **Stair-Step 2/2 Twill** (`stair-step-2-2`, atlas end index) — weftwise-elongated 2/2 (4×8): each classic 2/2 pick doubled so the diagonal reads as vertical stair-step blocks. **Why:** Match a reference drawdown for ENS-style stepped twill without remapping existing `p=` / tile-art indices (append-only).
